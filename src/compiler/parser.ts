...
const forEachChildTable: ForEachChildTable = {
    ...
    [SyntaxKind.JsxAttributes]: function forEachChildInJsxAttributes<T>(node: JsxAttributes, cbNode: (node: Node) => T | undefined, cbNodes?: (nodes: NodeArray<Node>) => T | undefined): T | undefined {
        return visitNodes(cbNode, cbNodes, node.properties) || visitNode(cbNode, node.staticBlock);
    },
    ...
    [SyntaxKind.JsxMethodAttribute]: function forEachChildInJsxMethodAttribute<T>(node: JsxMethodAttribute, cbNode: (node: Node) => T | undefined, cbNodes?: (nodes: NodeArray<Node>) => T | undefined): T | undefined {
        return visitNodes(cbNode, cbNodes, node.modifiers) ||
            visitNode(cbNode, node.name) ||
            visitNodes(cbNode, cbNodes, node.typeParameters) ||
            visitNodes(cbNode, cbNodes, node.parameters) ||
    ...
    [SyntaxKind.JsxRunDirective]: function forEachChildInJsxRunDirective<T>(node: JsxRunDirective, cbNode: (node: Node) => T | undefined, cbNodes?: (nodes: NodeArray<Node>) => T | undefined): T | undefined {
        return visitNodes(cbNode, cbNodes, node.statements);
    },
    ...
    [SyntaxKind.JsxLabeledFragment]: function forEachChildInJsxLabeledFragment<T>(node: JsxLabeledFragment, cbNode: (node: Node) => T | undefined, cbNodes?: (nodes: NodeArray<Node>) => T | undefined): T | undefined {
        return visitNode(cbNode, node.label) ||
            visitNodes(cbNode, cbNodes, node.parameters) ||
            visitNodes(cbNode, cbNodes, node.children);
    },
    [SyntaxKind.JsxStyleDirective]: function forEachChildInJsxStyleDirective<T>(node: JsxStyleDirective, cbNode: (node: Node) => T | undefined, cbNodes?: (nodes: NodeArray<Node>) => T | undefined): T | undefined {
        return visitNode(cbNode, node.name) ||
            visitNodes(cbNode, cbNodes, node.arguments) ||
            visitNode(cbNode, node.text);
    },
    [SyntaxKind.JsxPublicDeclaration]: function forEachChildInJsxPublicDeclaration<T>(node: JsxPublicDeclaration, cbNode: (node: Node) => T | undefined, cbNodes?: (nodes: NodeArray<Node>) => T | undefined): T | undefined {
        return visitNodes(cbNode, cbNodes, node.elements);
    },
...
namespace Parser {
    ...
    function canFollowExportModifier(): boolean {
        return token() === SyntaxKind.AtToken
            || (scriptKind === ScriptKind.Syn && token() === SyntaxKind.LessThanToken)
    ...
    function nextTokenIsIdentifierOrKeywordOrGreaterThanOrDotDotDot() {
        nextToken();
        return tokenIsIdentifierOrKeywordOrGreaterThan(token()) || token() === SyntaxKind.PrivateIdentifier || token() === SyntaxKind.DotDotDotToken;
    }

    function nextNumericTokenIsLikelyTaName() {
        if (nextToken() !== SyntaxKind.NumericLiteral) return false;
        if (scanner.getTokenFullStart() !== scanner.getTokenStart()) return false;
        if (nextToken() !== SyntaxKind.GreaterThanToken && token() !== SyntaxKind.Identifier) return false;
    ...
    function isListTerminator(kind: ParsingContext): boolean {
        ...
        switch (kind) {
            ...
            case ParsingContext.JsxAttributes:
                return token() === SyntaxKind.GreaterThanToken || token() === SyntaxKind.SlashToken || (token() === SyntaxKind.StaticKeyword && lookAhead(nextTokenIsOpenBraceOnSameLine));
            case ParsingContext.JsxRunDirectiveStatements:
    ...
    function parseBinaryExpressionRest(precedence: OperatorPrecedence, leftOperand: Expression, pos: number): Expression {
        while (true) {
            ...
            if (scriptKind === ScriptKind.Syn && token() === SyntaxKind.LessThanToken) {
                if (leftOperand.kind == SyntaxKind.JsxElement || leftOperand.kind == SyntaxKind.JsxSelfClosingElement)
                    break;
    ...
    function parseSimpleUnaryExpression(): UnaryExpression {
        switch (token()) {
            ...
            case SyntaxKind.AtToken: {
                nextToken();
                const node = parseJsxNode(/*inExpressionContext*/ true, /*topInvalidNodePosition*/ undefined, /*openingTag*/ undefined, /*mustBeUnary*/ true);
                (node as Mutable<typeof node>).flags |= NodeFlags.TargetedJsxElement;
                return node;
            }
            case SyntaxKind.LessThanToken:
    ...
    function parseUpdateExpression(): UpdateExpression {
        if (token() === SyntaxKind.PlusPlusToken || token() === SyntaxKind.MinusMinusToken) {
            ...
            if (lookAhead(() => nextTokenIsIdentifierOrKeywordOrGreaterThanOrDotDotDot() && !scanner.hasPrecedingLineBreak())) {
            ...
            if (lookAhead(nextNumericTokenIsLikelyTaName)) {
                // if we see something like `<3>` then parse as a tree node with a numeric tag name, preserving linked edits
                return parseJsxNode(/*inExpressionContext*/ true) as Node as MemberExpression;
            }
        }
    ...
    function parseJsxNode(inExpressionContext: boolean, topInvalidNodePosition?: number, openingTag?: JsxOpeningElement | JsxOpeningFragment, mustBeUnary?: boolean) {
        if (scriptKind === ScriptKind.Syn) {
            ...
            if (lookAhead(() => nextToken() === SyntaxKind.ColonToken)) {
                return parseJsxLabeledFragment(openingTag);
            }
        }
    ...
    function parseJsxElementOrSelfClosingElementOrFragment(inExpressionContext: boolean, topInvalidNodePosition?: number, openingTag?: JsxOpeningElement | JsxOpeningFragment, mustBeUnary = false): JsxElement | JsxSelfClosingElement | JsxFragment {
        ...
        if (!mustBeUnary && inExpressionContext && token() === SyntaxKind.LessThanToken && !lookAhead(() => nextToken() === SyntaxKind.PrivateIdentifier)) {
    ...
    function parseJsxElementIdentifier(): Identifier | undefined {
        if (!lookAhead(() => {
            parseExpected(SyntaxKind.AtToken);
            return token() === SyntaxKind.Identifier && scanner.getTokenFullStart() < scanner.getTokenStart();
        })) return;
        parseExpected(SyntaxKind.AtToken);
        const ident = parseIdentifier();
        parseOptionalJsxSemicolon();
        return ident;
    }

    function parseOptionalJsxSemicolon() {
        const pos = getNodePos();
        if (parseOptional(SyntaxKind.SemicolonToken) && scanner.hasPrecedingLineBreak()) {
            parseErrorAtPosition(pos, scanner.getTokenFullStart(), Diagnostics.Semicolons_should_not_terminate_a_line_inside_of_the_opening_tag);
        }
    }

    function parseJsxAttributes(hasClassList: boolean): JsxAttributes {
        const pos = getNodePos();
        const parseAttrs = () => parseList(ParsingContext.JsxAttributes, () => {
            const attr = parseJsxAttribute(hasClassList);
            parseOptionalJsxSemicolon();
            return attr;
        });
        const attrs = factory.createJsxAttributes(parseAttrs());
        if (token() === SyntaxKind.StaticKeyword) {
            (attrs as Mutable<typeof attrs>).staticBlock = parseStaticBlock();
        }
        if (token() !== SyntaxKind.SlashToken && token() !== SyntaxKind.GreaterThanToken && token() !== SyntaxKind.EndOfFileToken) {
            const invalidAttributes = parseAttrs();
            parseErrorAtPosition(invalidAttributes.pos, invalidAttributes.end, Diagnostics.A_static_block_must_be_the_last_attribute_in_the_opening_tag);
        }
        return finishNode(attrs, pos);
    }
    ...
    function parseJsxOpeningOrSelfClosingElementOrOpeningFragment(inExpressionContext: boolean): JsxOpeningElement | JsxSelfClosingElement | JsxOpeningFragment {
        ...
        if (scriptKind === ScriptKind.Syn && (token() === SyntaxKind.DotToken || token() === SyntaxKind.OpenParenToken)) {
            classList = parseJsxClassList();
        }

        if (classList && token() === SyntaxKind.AtToken) {
            const hasName = !!identifier;
            identifier = parseJsxElementIdentifier();
            if (!hasName && identifier) {
                parseErrorAtRange(identifier, Diagnostics.An_element_s_binding_name_must_appear_immediately_after_the_tag_name);
            } 
        }

        // FIXME: report a diagnostic if `@` and ident are parsed as attr
        // we need to report that element name binding should follow the tag name
    ...
    function parseJsxTagName(): Identifier | JsxNamespacedName | ThisExpression {
        const pos = getNodePos();
        scanJsxIdentifier();
        if (token() === SyntaxKind.NumericLiteral) { // error recovery
            if (lookAhead(() => nextToken() === SyntaxKind.Identifier && scanner.getTokenFullStart() === scanner.getTokenStart())) {
                const prefix = scanner.getTokenValue();
                nextToken();
                identifierCount++;
                const text = internIdentifier(prefix + scanner.getTokenValue());
                nextTokenWithoutCheck();
                return finishNode(factoryCreateIdentifier(text), pos);
            }
    ...
    function parseJsxExpression(inExpressionContext: boolean): JsxExpression | undefined {
        ...
        let staticToken: Token<SyntaxKind.StaticKeyword> | undefined;
        let dotDotDotToken: DotDotDotToken | undefined;
        let expression: Expression | undefined;
        if (token() !== SyntaxKind.CloseBraceToken) {
            staticToken = parseOptionalToken(SyntaxKind.StaticKeyword);
            if (!inExpressionContext) {
        ...
        const n = finishNode(factory.createJsxExpression(dotDotDotToken, expression), pos);
        if (staticToken) n.modifierFlagsCache |= ModifierFlags.Static;
        return n;
    }

    function parseJsxAttribute(hasClassList: boolean): JsxAttribute | JsxSpreadAttribute | JsxShorthandAttribute | JsxMethodAttribute {
        if (token() === SyntaxKind.OpenBraceToken) {
            if (scriptKind !== ScriptKind.Syn) {
        ...
        if (scriptKind === ScriptKind.Syn) {
            if (token() === SyntaxKind.AsyncKeyword && lookAhead(() => isJsxMethodAttributeStart(/*maybeAsync*/true))) {
                return parseJsxMethodAttribute();
            }
            if (lookAhead(isJsxMethodAttributeStart)) {
                return parseJsxMethodAttribute();
            }
        }
    ...
    function isJsxMethodAttributeStart(maybeAsync = false): boolean {
        if (maybeAsync) {
            nextToken();
            if (scanner.hasPrecedingLineBreak()) return false;
        }
        // skip attr name
        if (!isIdentifier() && !isKeyword(token())) return false;
        nextToken();
        if (token() === SyntaxKind.ColonToken) {
            nextToken();
            if (!isIdentifier() && !isKeyword(token())) return false;
    ...
    function parseJsxClassAttribute(insideParens = false, errorRecoveryStart?: number): JsxClassAttribute {
        ...
        while (
            ...
            lookAhead(() => tokenIsIdentifierOrKeyword(nextToken()))
        );
    ...
    function parseJsxClassList(): JsxClassList {
        ...
        if (token() === SyntaxKind.OpenParenToken) {
            ...
            while (token() !== SyntaxKind.EndOfFileToken) {
                ...
                if (token() === SyntaxKind.SlashToken || token() === SyntaxKind.GreaterThanToken) {
                    break;
                }
                if (token() === SyntaxKind.OpenBraceToken) {
            ...
            if (closed) parseExpected(SyntaxKind.CloseParenToken);
        }
        else {
            attributes = createNodeArray([parseJsxClassAttribute()], pos);
        }
        const list = finishNode(factory.createJsxClassList(attributes), pos);
        parseOptionalJsxSemicolon();
        return list;
    }
    ...
    function parseJsxMethodAttribute(): JsxMethodAttribute {
        const pos = getNodePos();
        let modifiers: NodeArray<Modifier> | undefined;
        if (token() === SyntaxKind.AsyncKeyword) {
            const modPos = getNodePos();
            const asyncMod = finishNode(factory.createToken(SyntaxKind.AsyncKeyword), modPos);
            nextToken();
            modifiers = createNodeArray([asyncMod], modPos);
        }
        const name = parseJsxAttributeName();
        const typeParameters = token() === SyntaxKind.LessThanToken ? parseTypeParameters() : undefined;
        parseExpected(SyntaxKind.OpenParenToken);
        const parameters = parseParametersWorker(SignatureFlags.Await, /*allowAmbiguity*/ true);
        parseExpected(SyntaxKind.CloseParenToken);
        const type = token() === SyntaxKind.ColonToken ? parseTypeAnnotation() : undefined;
        const body = parseBlock(/*ignoreMissingOpenBrace*/ false);
        return finishNode(factory.createJsxMethodAttribute(modifiers, name, typeParameters, parameters, type, body), pos);
    }
    ...
    function parseDirectiveEnd(tagName: Identifier | PrivateIdentifier, inExpressionContext: boolean) {
        ...
        else if (token() === SyntaxKind.LessThanSlashToken && lookAhead(() => nextToken() === SyntaxKind.PrivateIdentifier)) {
            nextToken();
            hasError = true;
            parseErrorAtCurrentToken(Diagnostics.Directive_tags_must_close_with_Slash);
            nextToken();
            if (parseExpected(SyntaxKind.GreaterThanToken, /*diagnosticMessage*/ undefined, /*shouldAdvance*/ false)) {
                if (inExpressionContext) {
                    nextToken();
    ...
    function parseJsxDirectiveBody(
        pos: number,
        tagName: Identifier | PrivateIdentifier,
    ...
    function parseJsxDirective(openingTag: JsxOpeningElement | JsxOpeningFragment | undefined) {
        ...
        switch (name) {
            ...
            case '#style': {
                const styleName = token() === SyntaxKind.Identifier ? parseIdentifier() : undefined;
                const styleArgs = token() === SyntaxKind.OpenParenToken ? parseArgumentList() : undefined;
                parseExpected(SyntaxKind.GreaterThanToken, /*diagnosticMessage*/ undefined, /*shouldAdvance*/ false);
                ...
                const { hasError } = parseDirectiveEnd(tagName, !openingTag);
                const n = finishNode(factory.createJsxStyleDirective(textNode, styleName, styleArgs), pos);
                if (hasError) (n as Mutable<typeof n>).flags |= NodeFlags.ThisNodeHasError;
                return n;
            }
            case '#component': {
                ...
                parseExpectedMatchingBrackets(SyntaxKind.OpenParenToken, SyntaxKind.CloseParenToken, openParenParsed, openParenPos);
                const type = parseReturnType(SyntaxKind.ColonToken, /*isType*/ false); // FIXME: remove this, we don't use return types
                ...
                const { children, hasError } = parseJsxDirectiveBody(pos, tagName, openingTag);
                const n = finishNode(factory.createJsxComponentDirective(undefined, asteriskToken, compName, typeParameters, parameters, type, body, children), pos);
                if (hasError) (n as Mutable<typeof n>).flags |= NodeFlags.ThisNodeHasError;
                return n;
            }
        }
        // error node
        (tagName as any as Mutable<Identifier>).kind = SyntaxKind.Identifier;
        const openEnd = lookAhead(() => nextToken() && scanner.getTokenFullStart());
        const open = finishNode(factory.createJsxOpeningElement(undefined, tagName as any as Identifier, undefined, undefined, finishNode(factory.createJsxAttributes([]), openEnd, openEnd), undefined), pos, openEnd);
        const { children, endPos } = parseJsxDirectiveBody(pos, tagName, openingTag);
        const ident = finishNode(factoryCreateIdentifier(""), endPos+2, endPos+2);
        const close = finishNode(factory.createJsxClosingElement(ident), endPos);
        return finishNode(factory.createJsxElement(open, children, close), pos);
    }

    function parseJsxComponentDirectiveDeclaration(_pos: number, modifiersIn: NodeArray<ModifierLike> | undefined): JsxComponentDirective {
        synFileContainsJsx = true;
        const directive = parseJsxDirective(/*openingTag*/ undefined) as JsxComponentDirective;
        const modifiers = modifiersIn as NodeArray<Modifier> | undefined;
        if (modifiers) {
            (directive as Mutable<JsxComponentDirective>).modifiers = modifiers;
        }
        return directive;
    }

    function parseJsxPublicDeclaration(): JsxPublicDeclaration {
        const pos = getNodePos();
        parseExpected(SyntaxKind.PublicKeyword);
        const elements = parseBracketedList(ParsingContext.ImportOrExportSpecifiers, parseExportSpecifier, SyntaxKind.OpenBraceToken, SyntaxKind.CloseBraceToken);
        return finishNode(factory.createJsxPublicDeclaration(elements), pos);
    }

    function parseJsxLabeledFragment(openingTag: JsxOpeningElement | JsxOpeningFragment | undefined) {
        const pos = getNodePos();
        parseExpected(SyntaxKind.LessThanToken);
        parseExpected(SyntaxKind.ColonToken);
        ...
        if (token() === SyntaxKind.OpenParenToken) {
            const openParenPos = getNodePos();
            const openParenParsed = parseExpected(SyntaxKind.OpenParenToken);
            parameters = parseParametersWorker(SignatureFlags.None, /*allowAmbiguity*/ true);
            parseExpectedMatchingBrackets(SyntaxKind.OpenParenToken, SyntaxKind.CloseParenToken, openParenParsed, openParenPos);
        }
        const { children, hasError } = parseJsxDirectiveBody(pos, label, openingTag);
        const n = finishNode(factory.createJsxLabeledFragment(label, parameters, children), pos);
        if (hasError) (n as Mutable<typeof n>).flags |= NodeFlags.ThisNodeHasError;
        return n;
    }
    ...
    function isUpdateStatement(): boolean {
        return lookAhead(() => {
            nextToken();
            if (token() == SyntaxKind.DotToken || token() === SyntaxKind.QuestionDotToken) return false;
            if (scanner.hasPrecedingLineBreak()) return false;
            return scanner.getTokenFullStart() !== scanner.getTokenStart();
        });
    }
    ...
    function parseUpdateBlockStatement(): UpdateBlockStatement {
        ...
        if (token() === SyntaxKind.SemicolonToken) {
            nextToken();
            return finishNode(factory.createUpdateBlockStatement(operands), pos);
        }
        if (token() !== SyntaxKind.OpenBraceToken && (scanner.hasPrecedingLineBreak() || token() === SyntaxKind.EndOfFileToken)) { // ASI-like behavior
            return finishNode(factory.createUpdateBlockStatement(operands), pos);
        }
        if (token() === SyntaxKind.CloseBraceToken) {
            return finishNode(factory.createUpdateBlockStatement(operands), pos);
    ...
    function nextTokenCanBeStaticTreeDeclaration() {
        switch (nextToken()) {
            case SyntaxKind.AsyncKeyword:
    ...
    function parseStaticBlock() {
        ...
        const block = parseBlock(/*ignoreMissingOpenBrace*/ false);
        (block as Mutable<typeof block>).pos = pos;
        block.modifierFlagsCache |= ModifierFlags.Static;
    ...
    function parseDeferStatement(): Statement {
        ...
        switch (lookAhead(nextToken)) {
            ...
            case SyntaxKind.MinusMinusToken:
                nextToken()
                return finishNode(factory.createDeferStatement(parseStatement(), isFinally), pos);
        }
    ...
    function isDeclaration(): boolean {
        while (true) {
            switch (token()) {
                ...
                case SyntaxKind.LessThanToken:
                    // export <#component ...> — JSX component directive as declaration
                    nextToken();
                    return token() === SyntaxKind.PrivateIdentifier && scanner.getTokenValue() === "#component";
    ...
    function isStartOfStatement(): boolean {
        switch (token()) {
            ...
            case SyntaxKind.StaticKeyword:
                if (lookAhead(nextTokenCanBeStaticTreeDeclaration) && (parsingContext & ParsingContext.JsxRunDirectiveStatements)) {
                    return true;
                }
            ...
            case SyntaxKind.PublicKeyword:
                ...
                if (lookAhead(() => nextToken() === SyntaxKind.OpenBraceToken) && inComponentDirectiveBody) {
                    return true;
                }
                // falls through
            case SyntaxKind.AccessorKeyword:
    ...
    function parseStatement(): Statement {
        switch (token()) {
            ...
            case SyntaxKind.StaticKeyword:
                if (parsingContext & (1 << ParsingContext.JsxRunDirectiveStatements)) {
                    if (lookAhead(() => nextToken() === SyntaxKind.OpenBraceToken)) {
            ...
            case SyntaxKind.PublicKeyword:
                if (lookAhead(() => nextToken() === SyntaxKind.OpenBraceToken) && inComponentDirectiveBody) {
                    return parseJsxPublicDeclaration();
                }
                // falls through
            case SyntaxKind.AsyncKeyword:
            case SyntaxKind.InterfaceKeyword:
            ...
            case SyntaxKind.LessThanToken:
                if (lookAhead(() => nextTokenIsIdentifierOrKeywordOrGreaterThanOrDotDotDot() && !scanner.hasPrecedingLineBreak())) {
                    const pos = getNodePos();
                    let hasJSDoc = hasPrecedingJSDocComment();
                    const jsx = parseJsxNode(/*inExpressionContext*/ true, /*topInvalidNodePosition*/ undefined, /*openingTag*/ undefined, /*mustBeUnary*/ true);
                    if (!tryParseSemicolon()) {
                        parseErrorForMissingSemicolonAfter(jsx);
                    }
                    const node = factoryCreateExpressionStatement(jsx);
                    return withJSDoc(finishNode(node, pos), hasJSDoc);
                }
                // falls through
        }
    ...
    function parseDeclarationWorker(pos: number, hasJSDoc: boolean, modifiersIn: NodeArray<ModifierLike> | undefined): Statement {
        switch (token()) {
            ...
            case SyntaxKind.LessThanToken:
                return parseJsxComponentDirectiveDeclaration(pos, modifiersIn) as unknown as Statement;
            case SyntaxKind.ExportKeyword:
                nextToken();
                switch (token()) {
                    case SyntaxKind.DefaultKeyword:
    ...
    function nextTokenIsOpenBraceOnSameLine() {
        return nextToken() === SyntaxKind.OpenBraceToken && !scanner.hasPrecedingLineBreak();
    }
