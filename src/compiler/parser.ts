...
namespace Parser {
    ...
    function parseBinaryExpressionRest(precedence: OperatorPrecedence, leftOperand: Expression, pos: number): Expression {
        while (true) {
            ...
            //   const y = foo()
            //   <#component Bar()>...
            if (token() === SyntaxKind.LessThanToken && scanner.hasPrecedingLineBreak() && scriptKind === ScriptKind.Syn && lookAhead(() => nextToken() === SyntaxKind.PrivateIdentifier)) {
                break;
            }
    ...
    function parseJsxChild(openingTag: JsxOpeningElement | JsxOpeningFragment | undefined, token: JsxTokenSyntaxKind): JsxChild | undefined {
        switch (token) {
            ...
            case SyntaxKind.JsxTextAllWhiteSpaces:
                return parseJsxText();
            case SyntaxKind.OpenBraceToken:
                if (scriptKind === ScriptKind.Syn && lookAhead(() => {
                    nextToken();
                    return scanner.hasPrecedingLineBreak();
                })) {
                    return parseJsxRunDirective();
                }
    ...
    function parseJsxRunDirective(): JsxRunDirective {
        const pos = getNodePos();
        parseExpected(SyntaxKind.OpenBraceToken);
        ...
        return finishNode(factory.createJsxRunDirective(statements), pos);
    }
